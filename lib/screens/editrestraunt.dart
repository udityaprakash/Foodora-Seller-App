...
import 'package:foodora_seller/config/api_links.dart';
...
class _EditrestrauntState extends State<Editrestraunt> {
  dynamic seller_details;
  @override
  void initState() {
    seller_details = sellerinfograbber();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: seller_details,
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          dynamic details = snapshot.data as dynamic;
          details = details['sellerDetails'];

          if (_restname.text.length == 0) {
            _restname.text = details['restaurantname'].toString();
          }
          if (_mobno.text.length == 0) {
            _mobno.text = details['mobilenumber'].toString();
          }
          if (_address.text.length == 0) {
            _address.text = details['restaurantaddress'].toString();
          }

          if (_timeo.text.length == 0) {
            _timeo.text = details['restaurant_openingtime'].toString();
          }
          if (_timec.text.length == 0) {
            _timec.text = details['restaurant_closingtime'].toString();
          }
                  ...
                  child: Column(
                    children: [
                      ...
                      InputFieldgenerator("Restraunt Name", context,
                          controller: _restname, check: (String Rest_name) {
                        restname = Rest_name;
                      }),
                      SizedBox(
                        height: MediaQuery.of(context).size.height / 35,
                        child: errortextgenerator(resnameerr,
